...
**`run_aemo_detectors_daily_post_tune.py`** applies the same three detectors to `aggregate_daily_demand(test)`, using the post-tuning configuration each detector was given by `run_fine_tune_on_synthetic.py`'s synthetic-only sweep rather than a plain class default, and matches against the region's *full* event catalogue — Tier 1 **and** Tier 2 — rather than Tier 1 only. Matching both tiers is what lets `produce_table_t2_daily_post_tune.py` report a Tier-1-specific unmatched count next to an overall one from a single run, instead of needing two differently-scoped runs to say both things. The detector receives only the processed test split, with no training or calibration warm-up. Its own `split_id` (`aemo_detect_daily_post_tune_v1`) and `train_samples=0` prevent these rows from being grouped with full-stream or adaptation results. Each of the three detectors is run and logged as its own, independent `record_run` call — their detections are never pooled or unioned before matching, so a detection one detector raised can never "help" another detector's match count.

An earlier `run_aemo_detectors_daily.py` (plain class defaults, Tier-1-only matching, `split_id="aemo_detect_daily_test_v1"`) served as the one-off sanity check for how much daily aggregation cuts detection volume, before daily aggregation was adopted as the adaptation arm's default detector input. That question is answered and recorded here; the script itself was removed once `run_aemo_detectors_daily_post_tune.py` existed to replace it as the actual source for Table T2. Its pre-tuning role was later revived as **`run_aemo_detectors_daily_pre_tune.py`** — same daily-aggregated input and class defaults, but both-tier matching this time (not Tier-1-only) and a new `split_id="aemo_detect_daily_pre_tune_v1"`, so it produces a genuine pre/post-tuning comparison against `run_aemo_detectors_daily_post_tune.py` using the same `tier1_unmatched`/`tier2_contextual` columns instead of a differently-shaped one-off check.

**`produce_figure_f2_aemo_daily_frozen.py` is a sibling of `produce_figure_f2_aemo.py`, not a replacement.** Redirecting the original script to `run_aemo_detectors_daily_post_tune.py`'s data instead of adding a new script would have orphaned `run_aemo_detectors_raw_pre_tune.py` — nothing else reads its `aemo_detect_raw_pre_tune_v1` rows. Both scripts and both their `runs.csv` sources are kept: the original still plots default-config detections against raw half-hourly demand from `run_aemo_detectors_raw_pre_tune.py`; the new one plots post-tuning-config, both-tier-matched detections against `aggregate_daily_demand` — the exact series the detector actually saw, not a separately-computed resample. Output filenames differ (`f2_detection_daily_frozen_<method>_<region>.png` vs. `f2_detection_<method>_<region>.png`) so neither run can overwrite the other's figures. Visual styling is identical between the two on purpose, so a reader can compare them directly — only the title text and data source differ.

**`run_aemo_detectors_raw_pre_tune.py` was later brought up to the same both-tier matching as `run_aemo_detectors_daily_post_tune.py`** — its `tier1_events()` helper became `region_events()`, matching the full catalogue (Tier 1 and Tier 2) instead of Tier 1 only, so its `runs.csv` rows carry the same `n_matched_t1`/`n_matched_t2`/`event_recall` breakdown. It's still the raw half-hourly, default-hyperparameter run (that hasn't changed) and it's still Figure F2's only source — but its `runs.csv` rows (`split_id="aemo_detect_raw_pre_tune_v1"`) now also feed `produce_table_t2_raw_pre_tune.py`, a second, read-only consumer of the same run rather than a new experiment, giving Table T2 a pre-tuning + raw-30-minute sibling.
...
**Unmatched detections get a full-height dashed line each (`ax.vlines`), matching Figure D and the matched-detection style, on purpose — even though this makes the raw-half-hourly-stream figures (`run_aemo_detectors_raw_pre_tune.py`, hundreds-to-thousands of false alarms) look like a solid red barcode that drowns out the demand curve.** A density-based fallback (short lines, or the old top-of-axis rug, past some detection-count threshold) was considered and explicitly declined — every figure uses the identical convention rather than switching styles depending on how many detections happen to fall in a given run. The daily-aggregated figures (`run_aemo_detectors_daily_post_tune.py`, tens of false alarms at most) are the ones this styling was designed to read well on; the raw-stream ones inherit it for consistency, not because they're expected to be legible at that density.

**Three detection scripts were renamed so pre-tuning vs. post-tuning is visible in the filename, not just in a docstring:** `run_detector_budget_tuning.py` → `run_fine_tune_on_synthetic.py`, `run_synthetic_detector_experiments.py` → `run_post_tune_on_synthetic.py`, `run_detector_input_experiments.py` → `run_post_tune_input_comparison_on_aemo.py`. `run_fine_tune_on_synthetic.py`'s own output moved with it, from `results/tables/detector_budget_tuning.csv` to `results/tables/fine_tune_on_synthetic.csv` — nothing else read the old name. `run_post_tune_on_synthetic.py` additionally stopped writing `results/tables/frozen_synthetic_detector_results.csv` directly; it now only calls `record_run()`, the same as `run_pre_tune_on_synthetic.py` (its pre-tuning sibling) always has, and a `produce_table_t1_post_tune.py` script builds the report table from `runs.csv` instead — a produce script, not the run script, owns turning `runs.csv` rows into a `results/tables/*.csv`.

**`produce_table_t1_pre_tune.py` had a real bug: its `groupby(["method", "drift_type"])` carried no `split_id` filter at all.** Once `run_post_tune_on_synthetic.py`'s frozen-config rows (`split_id="synthetic_full_series_20000_observations"`) existed in `runs.csv` alongside `run_pre_tune_on_synthetic.py`'s pre-tuning rows (`split_id="synth_n20000_cp*"`, one per drift type), every group silently pooled both experiments' five seeds together — a `delay (mean ± sd)` and `missed` count blended across two different detector configs, with `threshold` showing whichever config's `config_hash` happened to appear first in the group. This was never caught because `seeds` still read 5 (both experiments reuse the same `SEEDS` list, so `nunique()` couldn't tell 5-from-one-experiment apart from 5-plus-5-from-two). Fixed by splitting into two explicitly-pinned tables — `produce_table_t1_pre_tune.py` (pre-tuning, `split_id.str.startswith("synth_n20000_cp")`) and `produce_table_t1_post_tune.py` (post-tuning, exact `split_id` match) — sharing all column/aggregation logic through `table_t1_common.py` so the two tables can't drift apart in how a cell is computed, only in which rows feed them. The shared dedup logic also had to key on `(method, dataset, seed)`, not `(method, dataset)`: both synthetic experiments call `record_run()` once *per seed*, so a `(method, dataset)`-only "latest timestamp" dedup (the pattern `produce_table_t2_daily_post_tune.py` uses, valid there because its runs call `record_run()` once per (detector, region) covering every metric) would have kept only whichever single seed ran most recently and silently dropped the other four.

**Table T2 gained two pre-tuning siblings, `produce_table_t2_daily_pre_tune.py` and `produce_table_t2_raw_pre_tune.py`, both sharing `produce_table_t2_daily_post_tune.py`'s columns and aggregation via `table_t2_common.py`.** The daily-aggregated one reads the new `run_aemo_detectors_daily_pre_tune.py`; the raw one reads `run_aemo_detectors_raw_pre_tune.py`'s existing `aemo_detect_raw_pre_tune_v1` rows (already both-tier matched — see above) rather than a new experiment. Together with the canonical post-tuning table, this gives three T2 variants spanning both axes independently: input processing (raw vs. daily-aggregated) and tuning stage (pre vs. post) — e.g. the raw + pre-tuning table shows near-zero Tier 1 misses but precision near 0.01-0.04 (default hyperparameters fire on almost every half-hourly fluctuation), which is the concrete failure mode daily aggregation and tuning together are meant to fix.

**Every `produce_table_*.py` script now also renders a styled `.png` table image, not just the `.csv`, via a shared `table_image.py` helper.** Matplotlib's `ax.table()` with `loc="center"` and no explicit `bbox` sizes and centers the table using its own content-derived height, inside axes that still span the whole figure — even with `axis("off")`, that leaves a large blank gap above/below once `bbox_inches="tight"` crops the saved PNG, because the invisible axes still contributes to the crop's bounding box. `table_image.py` fixes this by giving the table an explicit `bbox=(0, 0, 1, 1)` inside an axes sized to exactly its row count, with title/subtitle drawn via `fig.text` in a separately reserved strip above it, so the rendered figure height matches what was actually computed for it.

**`run_post_tune_on_synthetic.py` had its own bespoke `save_changepoints()`, writing to a new top-level `results/changepoints/` directory — a second, one-off persistence mechanism the codebase didn't need.** `results_io` already has a config_hash-keyed convention for exactly this (`dump_synthetic_detections()`/`synthetic_detections_path()` for synthetic runs, `dump_detections()`/`detections_path()` for AEMO), and `run_pre_tune_on_synthetic.py` (this script's pre-tuning sibling) already used it. `save_changepoints()` additionally persisted the true changepoints and a matched/false-alarm classification per point — but both are cheap to recompute (`make_series()` is deterministic given `(kind, seed)`, and the classification comes straight out of the `evaluate_detections()` result already sitting in memory in the same loop iteration that calls `detect()`), so none of it actually needed to survive a run. Fixed by switching to `dump_synthetic_detections()` (just the detected indices, same file layout `run_pre_tune_on_synthetic.py` produces) and having `plot_synthetic_run()` (called in the same loop, no round trip needed at all) take the series/true changepoints/evaluation dict directly instead of reading a CSV back. `produce_figure_f2_synthetic.py` (the one other consumer, later split into `produce_figure_f2_synthetic_pre_tune.py` / `produce_figure_f2_synthetic_post_tune.py` -- see below) was updated to read `synthetic_detections_path(config_hash, dataset, seed)` instead of the retired `results/changepoints/*.csv` — it never used the persisted classification either (each `plot_*` there already recomputed its own matched/unmatched split from the true changepoint constants and a tolerance window), only the plain list of detected indices. `results/changepoints/` was deleted; nothing else read it.

**`produce_figure_f2_synthetic.py` was split into a pre-tuning/post-tuning pair and moved into `experiments/produce/`, matching every other producer.** It previously lived at the top level of `experiments/` (an inconsistency with the `produce_figure_f2_aemo*.py` pair, which are both under `experiments/produce/`) and only ever read `run_post_tune_on_synthetic.py`'s rows at a single hardcoded seed, with three separate `plot_sudden`/`plot_gradual`/`plot_recurring` functions that each hand-rolled their own tolerance-window matching and drift-type-specific annotation layout — none of which reused `evaluation.evaluate_detections()`, so a delay shown on this figure could in principle drift from what Table T1 reports. Replaced by `figure_f2_synthetic_common.py` (shared panel/figure logic), `produce_figure_f2_synthetic_pre_tune.py` (reads `run_pre_tune_on_synthetic.py`'s `"synth_n20000_cp*"` rows), and `produce_figure_f2_synthetic_post_tune.py` (reads `run_post_tune_on_synthetic.py`'s fixed split_id) — one figure per drift type with an actual changepoint (`none` has nothing to mark), one panel per seed instead of a single hardcoded seed, and matching now goes through `evaluate_detections()` directly so a match/delay shown here is always the same one T1 aggregates. Output filenames changed from `synthetic_{kind}_detection.png` to `f2_synthetic_{pre_tune,post_tune}_{kind}.png`; the old files were deleted.

**`run_post_tune_on_synthetic.py`'s own `plot_synthetic_run()` (one PNG per (detector, drift_type) at seed 1, `synthetic_{kind}_{detector}_seed1.png`) was removed once the composite `produce_figure_f2_synthetic_post_tune.py` figures existed** — the composite figure already shows all three detectors, all five seeds, and each match's delay in one place per drift type, so the single-seed single-detector plot was pure duplication that would have kept regenerating on every run otherwise. `run_post_tune_on_synthetic.py` now only logs to `runs.csv` and dumps detections; it does not plot anything itself.

**Table T1's post-tuning table, `produce_table_t1_post_tune.py`, was briefly retired (deleted, along with `table_t1_post_tune_synthetic_detection.{csv,png}`), then recreated.** The stated reason for retiring it stood on its own: aggregate synthetic-benchmark delay/false-alarm numbers for the post-tuning configs, in isolation, had nothing in the same table to compare against. It came back once the goal shifted from "is this one table useful on its own" to "does a systematic pre-tuning/post-tuning x Table-1/Figure-2 matrix exist for the synthetic benchmark" — recreating it, alongside its already-existing `produce_table_t1_pre_tune.py` sibling and both `produce_figure_f2_synthetic_*.py` figures, completes that four-cell matrix. `produce_table_t2_all_streams_post_tune.py` (below) still separately covers post-tuning detector performance against real AEMO events.

**`produce_table_t2_all_streams_post_tune.py` combines all three post-tuning input streams (raw, daily-aggregated, deseasonalized) into one table**, with an added `input_stream_type` column, by calling `table_t2_common.build_table()` once per split_id and concatenating -- a pure reshape, no new metric. Doing this required a new experiment first: `run_aemo_detectors_raw_post_tune.py`. `run_post_tune_input_comparison_on_aemo.py` (at the time) already ran post-tuning detectors on raw demand, but its `tier1_events()` helper matched Tier 1 events only, so its `tier2_contextual`/`unmatched` figures weren't on the same footing as the daily-aggregated and deseasonalized rows (both matched against the *full* Tier 1 + Tier 2 catalogue via `region_events()`). `run_aemo_detectors_raw_post_tune.py` mirrors `run_aemo_detectors_raw_pre_tune.py` exactly (same raw input, same warm-up-then-test-window scoring, same both-tier matching) but with post-tuning configs and its own `split_id="aemo_detect_raw_post_tune_v1"`, so all three rows in the combined table are comparable. This table is additive — it does not replace or overwrite `produce_table_t2_daily_post_tune.py` or either of its single-input siblings.

**The full (input stream x tuning stage) matrix -- 3 streams (raw, daily-aggregated, deseasonalized) x 2 stages (pre-tuning, post-tuning) = 6 cells -- was completed for both Table T2 and Figure F2, after a scan turned up three real gaps: no `run_aemo_detectors_deseasonalized_pre_tune.py` (and so no Table T2 cell for it), no `produce_table_t2_raw_post_tune.py` single-input table (the combined post-tuning table existed, but not the standalone one matching its five siblings), and only 2 of the 6 possible AEMO Figure F2 producers existed at all.** Filled by: `run_aemo_detectors_deseasonalized_pre_tune.py` (mirrors `run_aemo_detectors_deseasonalized_post_tune.py` with class-default configs, `split_id="aemo_detect_deseasonalized_pre_tune_v1"`); `produce_table_t2_raw_post_tune.py` and `produce_table_t2_deseasonalized_pre_tune.py` (the two missing single-input Table T2 cells, both trivial `table_t2_common.build_table()` calls); and `produce_table_t2_all_streams_all_tuning.py`, which extends `produce_table_t2_all_streams_post_tune.py`'s pattern across all six cells with two new columns, `input_stream_type` and a `fine_tuned` boolean (kept as a sibling, not a replacement, since the post-tuning-only table is still the narrower, currently-more-relevant view).

**The two pre-existing AEMO Figure F2 scripts, `produce_figure_f2_aemo.py` and `produce_figure_f2_aemo_daily_frozen.py`, were ~95% duplicate code (only the split_id, the daily-aggregation step, one annotation branch, the subtitle, and the output filename differed) and both used generic, non-descriptive names -- the same "canonical file with no tag" problem fixed earlier for `produce_table_t1.py`/`produce_table_t2.py`.** Replaced by `figure_f2_aemo_common.py` (all shared plotting logic: palette, event overlays, matched/unmatched detection markers, legend, footer stats) and six explicitly-named producers, one per (stream, stage) cell: `produce_figure_f2_aemo_{raw,daily,deseasonalized}_{pre_tune,post_tune}.py`. What varies per producer is now just four parameters passed to `figure_f2_aemo_common.build_all_figures()`: `split_id`, which series to display for visual context (the detector's own daily-aggregated input verbatim for `daily`, versus a daily-mean resample of a too-dense-to-plot half-hourly stream for `raw` and `deseasonalized`), the y-axis label, and the output filename prefix. Output filenames changed from `f2_detection_<method>_<region>.png` / `f2_detection_daily_frozen_<method>_<region>.png` to `f2_detection_{raw,daily,deseasonalized}_{pre_tune,post_tune}_<method>_<region>.png` for all six; the two old scripts and their old-named figures were deleted.

**`run_post_tune_input_comparison_on_aemo.py` was retired entirely** (deleted, along with its output `results/tables/aemo_detector_input_comparison.csv` and its six `{region}_{raw_30min,daily_mean_complete_days,daily_weekly_adjusted}_final_changepoints.png` figures). It predated the dedicated single-input post-tuning scripts and covered the same three input streams in one run, but only ever matched Tier 1 events, and its detection-density/gap/runtime columns (`detections_per_1000_observations`, `median_detection_gap`, `wall_clock_s`, ...) had no equivalent elsewhere. Once `run_aemo_detectors_raw_post_tune.py` existed alongside the daily and deseasonalized siblings, every input stream it covered had a both-tier-matched, individually-pinnable replacement feeding `produce_table_t2_all_streams_post_tune.py`, and it was judged not worth keeping around for the density/timing columns alone. `results/runs.csv` still has its historical rows under `split_id="aemo_detector_input_v1"` (the ledger is append-only and isn't scrubbed), but nothing reads them anymore.
...
- **Figure scripts read saved curve files.** `produce_figure_f1.py` reads the exact rolling-MAE curve saved by the experiment through `results_io.dump_curve()`. The path is keyed by `config_hash`, `dataset`, `region`, and `seed`, and `results_io.curve_path()` creates the same path on both the write and read sides. The figure therefore does not recalculate rolling MAE from forecasts. `produce_table_t1_pre_tune.py` only groups scalar values already stored in `runs.csv`.
