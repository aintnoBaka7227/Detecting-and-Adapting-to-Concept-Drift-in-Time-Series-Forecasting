"""AEMO detection on the standard-half-hourly stream, pre-tuning (class
default) configs.

"standard-half-hourly" = raw half-hourly demand, deseasonalised
(TRAIN-fitted day-of-year x weekday/half-hour profile removed) and
standardised (z-scored on TRAIN residual mean/std only) -- see
standard_stream_common.py for the shared preprocessing. The detector
runs continuously across TRAIN then Calibration then TEST (warm-up), and
only TEST-period alarms are scored, matching
run_aemo_detectors_raw_pre_tune.py's convention.

Sibling of run_aemo_detectors_standard_half_hourly_post_tune.py:
identical input stream and both-tier (Tier 1 and Tier 2) matching, but
plain `ADWINDetector()` / `KSWINDetector()` / `PageHinkleyDetector()`
defaults instead of the post-tuning, synthetic-tuned (half-hourly-cadence)
configs.

split_id "aemo_detect_standard_half_hourly_pre_tune_v1": its own id,
distinct from every other (stream, tuning-stage) combination.
"""
