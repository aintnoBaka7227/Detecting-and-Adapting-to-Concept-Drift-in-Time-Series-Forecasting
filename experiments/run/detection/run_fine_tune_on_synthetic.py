...
Tuned separately for two deployment cadences, since "false alarms per
year" depends on how many samples make up one year for the stream a
frozen config will eventually watch:

    SAMPLES_PER_YEAR = {"daily": 365, "half_hourly": 48 * 365}

Detector parameters and winners are never reused between cadences -- the
full grid is swept independently for "daily" and "half_hourly", even
though the underlying synthetic series are identical between the two
(only the false-alarms-per-year annualisation differs).
...
Eligibility per (detector configuration, cadence) -- checked per row,
across every detector, every seed and both cadences, not as an average:
...
Winner selection per (detector, cadence), among eligible configs only:
...
A (detector, cadence) with no eligible configuration is recorded with
status "no_eligible_configuration" rather than a failing fallback.

Runtime: a fresh detector instance is created for every single
(cadence, configuration, kind, seed) combination -- detector state is
never reused across calls, even though the two cadences' detection
results are numerically identical (only their annualised rate differs).
Within one (cadence, configuration) unit, all five no-drift seeds run
first; a unit that fails `none_clean` skips its drift scenarios
entirely, and a unit stops at its first missed drift or first
...
that was active when it was written, so a rule change (like this one)
requires clearing old checkpoints, not just re-running.
...
CADENCES = tuple(SAMPLES_PER_YEAR)
...
def false_alarms_per_year(n_false_alarms: int, n_observations: int, cadence: str) -> float:
    return (
        n_false_alarms
        / n_observations
        * SAMPLES_PER_YEAR[cadence]
    )
...
def full_config(cadence: str, kwargs: dict) -> dict:
    """Detector kwargs plus cadence/samples_per_year -- the identity a
    config_hash is computed from, so the same hyperparameters tuned under
    two different cadences never collide into one config_hash."""
    return {**kwargs, "cadence": cadence, "samples_per_year": SAMPLES_PER_YEAR[cadence]}


def _run_one(detector_class, kwargs: dict, kind: str, seed: int, cadence: str) -> dict:
    ...
    return {
        ...
        "false_alarms_per_year": false_alarms_per_year(n_false, len(series), cadence),
...
def evaluate_unit(cadence: str, sweep_name: str, detector_class, kwargs: dict) -> dict:
    ...
    Runtime shortcuts applied here, in order:
      ...
      2. if the mean no-drift false-alarms/year exceeds the budget,
         drift scenarios are skipped entirely for this unit;
      3. otherwise drift scenarios run kind by kind, seed by seed, and
         stop at the first missed drift, since one miss already fails
         the "total missed drifts == 0" rule regardless of what the
         remaining seeds/kinds would have shown.
    """
    ...
    none_fa_per_year = []
    for seed in SEEDS:
        row = _run_one(detector_class, kwargs, "none", seed, cadence)
        rows.append(row)
    ...
    if none_clean:
        for kind in DRIFT_KINDS:
            if stopped_early:
                break
            for seed in SEEDS:
                row = _run_one(detector_class, kwargs, kind, seed, cadence)
                rows.append(row)
                # The <=2/year budget applies per row here -- every
                # detector, every seed, every kind, every cadence -- not
                # just as a mean over the no-drift seeds. One missed
                # drift or one over-budget row already rules the config
                # out, so evaluation stops at the first of either.
                if (
...
def log_unit_to_runs_csv(config_hash: str, config: dict, unit: dict) -> None:
    ...
    for row in unit["rows"]:
        record_run(
            ...
            split_id=f"synth_{cadence}_n{N}_{row['kind']}",
...
def select_winners(completed_units: dict[str, dict]) -> pd.DataFrame:
    """One winner per (detector, cadence) among eligible units only. Pure
    function of `completed_units`, so re-running it on the same input is
    the determinism check `validate_before_saving` relies on."""

    by_method_cadence: dict[tuple[str, str], list[dict]] = {}
    for config_hash, unit in completed_units.items():
        ...
        key = (unit["method"], unit["cadence"])
        by_method_cadence.setdefault(key, []).append(
            {
    ...
    for method in DETECTOR_NAMES:
        for cadence in CADENCES:
            candidates = by_method_cadence.get((method, cadence), [])
...
def validate_before_saving(
    ...
    assert set(full_results["cadence"]) == set(CADENCES), "both cadences must be present"
    ...
    for _, winner in selected.iterrows():
        ...
        assert kinds_present == set(KINDS), (
            f"winner {winner['detector']}/{winner['cadence']} is missing kinds "
            f"{set(KINDS) - kinds_present}, so it cannot truly be eligible"
        )
        assert all(row["missed_detections"] == 0 for row in unit["rows"]), (
            f"winner {winner['detector']}/{winner['cadence']} has a missed drift"
        )
        assert all(
            row["n_false_alarms"] == 0 for row in unit["rows"] if row["kind"] == "none"
        ), (
            f"winner {winner['detector']}/{winner['cadence']} has a false alarm on "
            "the no-drift stream"
        )
        assert all(
            ...
            f"winner {winner['detector']}/{winner['cadence']} exceeds the false-alarm "
            "budget on at least one row"
        )
...
def main() -> None:
    candidates = [
        (cadence, sweep_name, detector_class, kwargs)
        for cadence in CADENCES
        for sweep_name, detector_class, kwargs in candidate_sweeps()
    ]

    completed_units: dict[str, dict] = {}
    pending = []

    for cadence, sweep_name, detector_class, kwargs in candidates:
        config = full_config(cadence, kwargs)
        ...
        if cached is not None:
            ...
            pending.append((config_hash, config, cadence, sweep_name, detector_class, kwargs))

    if pending:
        max_workers = max(1, (os.cpu_count() or 2) - 1)
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(evaluate_unit, cadence, sweep_name, detector_class, kwargs): (
                    config_hash,
                    config,
                )
                for config_hash, config, cadence, sweep_name, detector_class, kwargs in pending
            }
