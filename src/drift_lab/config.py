...
# Team-agreed seed list — every experiment this semester uses these five, so
# results are comparable across every table.
SEEDS = (1, 2, 3, 4, 5)
...
# Width of the "drift" regime band around a changepoint, in observations.
# 7 days of half-hourly data. An assumption, not a measured quantity — widen
# it if detectors on AEMO turn out to need a longer settling window.
REGIME_DRIFT_MARGIN = 48 * 7

DOCUMENTED_EVENTS_CSV = ROOT_DIR / "src" / "drift_lab" / "aemo" / "events.csv"